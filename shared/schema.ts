...
// =============================================================================
// EXERCISE DATABASE - For video integration and AI workout generation
// =============================================================================

export const exercises = pgTable("exercises", {
  id: serial("id").primaryKey(),
  name: text("name").notNull().unique(),
  category: text("category").notNull(), // upper-body, lower-body, core, cardio, full-body
  muscleGroups: text("muscle_groups").notNull(), // JSON array: ["chest", "triceps"]
  equipment: text("equipment").notNull(), // bodyweight, dumbbells, barbell, machine, etc.
  difficulty: text("difficulty").notNull(), // beginner, intermediate, advanced
  videoUrl: text("video_url"), // URL to video file (Google Drive or CDN)
  thumbnailUrl: text("thumbnail_url"), // Thumbnail image URL
  description: text("description"), // How to perform the exercise
  instructions: text("instructions"), // Step-by-step instructions (JSON array)
  tips: text("tips"), // Pro tips for better form
  commonMistakes: text("common_mistakes"), // What to avoid
  alternatives: text("alternatives"), // JSON array of alternative exercise IDs
  isCompound: boolean("is_compound").default(false), // Compound vs isolation
  caloriesPerMinute: integer("calories_per_minute"), // Estimated calories burned
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
});
...
// Workout-Exercise junction table (for tracking sets, reps, weight in completed workouts)
export const workoutExercises = pgTable("workout_exercises", {
  id: serial("id").primaryKey(),
  workoutId: text("workout_id").notNull(), // References workout ID from local storage
  exerciseId: integer("exercise_id").notNull().references(() => exercises.id),
  sets: integer("sets").notNull(),
  reps: text("reps").notNull(), // Can be "8-10" or "12" or "60 sec"
  weight: integer("weight"), // Weight used (in lbs or kg)
  restTime: integer("rest_time").notNull(), // Rest time in seconds
  completed: boolean("completed").default(false),
  notes: text("notes"), // User notes for this exercise
  createdAt: timestamp("created_at").notNull().defaultNow(),
});
