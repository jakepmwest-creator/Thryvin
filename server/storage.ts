...
// Using memory storage for immediate functionality - database was too unreliable
// Simple file-based storage for persistent accounts
class FileStorage implements IStorage {
  ...
  private async saveUsers() {
    try {
      ...
      console.log('💾 User accounts saved to disk!');
    } catch (error) {
  ...
  async createUser(userData: InsertUser): Promise<User> {
    const user: User = {
      id: this.nextId++,
      ...userData,
      trialEndsAt: userData.trialEndsAt || new Date(Date.now() + 14 * 24 * 60 * 60 * 1000),
      hasActiveSubscription: userData.hasActiveSubscription ?? false,
      weeklyGoalWorkouts: userData.weeklyGoalWorkouts ?? 5,
      weeklyGoalMinutes: userData.weeklyGoalMinutes ?? 150,
  ...
  // Stub implementations for other methods (not needed for basic auth)
  ...
  async getWorkoutsByType(type: string): Promise<Workout[]> { return []; }
  async getWorkout(id: number): Promise<Workout | undefined> { return undefined; }
  async getUserWorkouts(userId: number): Promise<UserWorkout[]> { return []; }
  async createUserWorkout(userWorkout: InsertUserWorkout): Promise<UserWorkout> { throw new Error('Not implemented'); }
  async getUserCompletedWorkoutsThisWeek(userId: number): Promise<number> { return 0; }
  async getUserTrainingMinutesThisWeek(userId: number): Promise<number> { return 0; }
  async savePerformanceLog(performanceData: InsertPerformanceLog): Promise<PerformanceLog> { throw new Error('Not implemented'); }
  async getPerformanceHistory(userId: number, exerciseId?: string, limit?: number): Promise<PerformanceLog[]> { return []; }
  async getPerformanceInsights(userId: number): Promise<any> { return {}; }
  async getAchievements(): Promise<Achievement[]> { return []; }
  async getAchievementsByCategory(category: string): Promise<Achievement[]> { return []; }
  ...
  async checkAndAwardAchievements(userId: number): Promise<Achievement[]> { return []; }
  async markAchievementsAsDisplayed(userId: number): Promise<void> { }
  async getQuests(): Promise<Quest[]> { return []; }
  async getQuestsByType(type: string): Promise<Quest[]> { return []; }
  async getQuest(id: number): Promise<Quest | undefined> { return undefined; }
  async getUserQuests(userId: number): Promise<(UserQuest & { quest: Quest })[]> { return []; }
  async createUserQuest(userQuest: InsertUserQuest): Promise<UserQuest> { throw new Error('Not implemented'); }
  async updateUserQuestProgress(userId: number, questId: number, progress: number): Promise<UserQuest | undefined> { return undefined; }
  async claimQuestReward(userId: number, questId: number): Promise<UserQuest | undefined> { return undefined; }
  async calculateWorkoutStreak(userId: number): Promise<number> { return 0; }
  async getUserProgressSnapshots(userId: number, period: string): Promise<ProgressSnapshot[]> { return []; }
  async createProgressSnapshot(snapshot: InsertProgressSnapshot): Promise<ProgressSnapshot> { throw new Error('Not implemented'); }
  async getLatestProgressSnapshot(userId: number, period: string): Promise<ProgressSnapshot | undefined> { return undefined; }
  async getNutritionProfile(userId: number): Promise<NutritionProfile | undefined> { return undefined; }
  async createNutritionProfile(profile: InsertNutritionProfile): Promise<NutritionProfile> { throw new Error('Not implemented'); }
  async updateNutritionProfile(userId: number, updates: Partial<NutritionProfile>): Promise<NutritionProfile | undefined> { return undefined; }
  async getUserMealPlans(userId: number): Promise<MealPlan[]> { return []; }
  async getMealPlan(id: number): Promise<MealPlan | undefined> { return undefined; }
  async createMealPlan(mealPlan: InsertMealPlan): Promise<MealPlan> { throw new Error('Not implemented'); }
  async getUserLoggedMeals(userId: number, date?: string): Promise<LoggedMeal[]> { return []; }
  async createLoggedMeal(loggedMeal: InsertLoggedMeal): Promise<LoggedMeal> { throw new Error('Not implemented'); }
  async getDailyNutritionStats(userId: number, date: string): Promise<{ calories: number, protein: number, carbs: number, fat: number }> { 
    return { calories: 0, protein: 0, carbs: 0, fat: 0 };
  }
  async getPosts(): Promise<Post[]> { return []; }
  ...
  async updatePost(id: number, updates: Partial<Post>): Promise<Post | undefined> { return undefined; }
  async deletePost(id: number): Promise<void> {}
  async getPostComments(postId: number): Promise<Comment[]> { return []; }
  async createComment(comment: InsertComment): Promise<Comment> { throw new Error('Not implemented'); }
  async deleteComment(id: number): Promise<void> {}
  async likePost(postId: number, userId: number): Promise<PostLike> { throw new Error('Not implemented'); }
  async unlikePost(postId: number, userId: number): Promise<void> {}
  async isPostLikedByUser(postId: number, userId: number): Promise<boolean> { return false; }
  async followUser(followerId: number, followingId: number): Promise<UserFollow> { throw new Error('Not implemented'); }
  async unfollowUser(followerId: number, followingId: number): Promise<void> {}
  async getUserFollowers(userId: number): Promise<User[]> { return []; }
  async getUserFollowing(userId: number): Promise<User[]> { return []; }
  async isUserFollowing(followerId: number, followingId: number): Promise<boolean> { return false; }
  async getMilestones(): Promise<Milestone[]> { return []; }
  async getUserMilestones(userId: number): Promise<Milestone[]> { return []; }
  ...
  async getMilestoneComments(milestoneId: number): Promise<MilestoneComment[]> { return []; }
  async createMilestoneComment(comment: InsertMilestoneComment): Promise<MilestoneComment> { throw new Error('Not implemented'); }
  async deleteMilestoneComment(id: number): Promise<void> {}
  async likeMilestone(milestoneId: number, userId: number): Promise<MilestoneLike> { throw new Error('Not implemented'); }
  async unlikeMilestone(milestoneId: number, userId: number): Promise<void> {}
