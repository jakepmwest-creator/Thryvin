...
Respond with a complete workout in JSON format:
{
  ...
  // POST /api/v1/workouts/generate-week - Generate the whole week (Mon–Sun, stub, no AI)
  app.post("/api/v1/workouts/generate-week", async (req: Request, res) => {
    if (!req.user) {
      return res.status(401).json({ error: "Not authenticated" });
    }
    ...
    try {
      ...
      console.log("📊 workout_days table ensured to exist for generate-week");
      ...
      // For each day of the week, ensure a row exists with status="pending"
      for (const date of weekDates) {
        ...
        // Only create if doesn't exist (don't delete existing rows)
        if (!existingWorkout) {
          ...
          console.log(`📅 Created workout day for ${date} with status=pending`);
        } else {
          console.log(
            `📅 Workout day for ${date} already exists with status=${existingWorkout.status}`,
          );
        }
      }

      return res.json({ dates: weekDates });
    } catch (error) {
      console.error("Error generating week workouts:", error);
      return res
        .status(500)
        .json({ error: "Failed to generate week workouts" });
    }
  });
